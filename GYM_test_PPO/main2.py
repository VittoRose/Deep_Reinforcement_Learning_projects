...
BATCH_SIZE = n_env*n_step       # 4*128 data collected for each update

LR = 2.5e-4
MAX_ITERATION = 1_000_000
GAMMA = 0.99
GAE_LAMBDA = 0.99
K_EPOCHS = 4        # update_epochs in original code
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, n_step):
            ...
            for i in range(len(done)):
                if done[i]:
                    print(f"Enviroment {i} terminated with a total reward: {int(ep_reward[i])}")
        ...
        for epoch in range(K_EPOCHS):
            ...
            for start in range(0, BATCH_SIZE, MINI_BATCH_SIZE):     # from 0 to batch_size, minibatch step
                ...
                pg_loss = torch.max(surr1, surr2).mean()                # TODO: Change max into min when working

                # Value loss
                # TODO: add clipped value loss

                v_losses = torch.nn.functional.mse_loss(newval, b_returns[mini_batch_index])
